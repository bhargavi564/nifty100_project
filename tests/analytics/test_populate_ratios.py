...
class TestPopulateRatios(unittest.TestCase):
    ...
    @classmethod
    def setUpClass(cls):
        # Force testing connections to look at the exact root database path
    ...
    def test_row_count(self):
        df = pd.read_sql(
            "SELECT COUNT(*) AS total FROM financial_ratios", self.conn
        )
    ...
    def test_company_id_column(self):
        df = pd.read_sql(
            "SELECT company_id FROM financial_ratios LIMIT 5", self.conn
        )
    ...
    def test_year_column(self):
        df = pd.read_sql(
            "SELECT year FROM financial_ratios LIMIT 5", self.conn
        )
    ...
    def test_debt_equity_column(self):
        df = pd.read_sql(
            "SELECT debt_to_equity FROM financial_ratios LIMIT 5", self.conn
        )
